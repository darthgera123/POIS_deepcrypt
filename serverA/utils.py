...
def dgk_init_without_route(regen):
    ...
    if regen:
        current_app.config['dgk_priv'], current_app.config['dgk_pub'] = dgk.keygen(
            2048, 160, 18, "./")
    else:
        current_app.config['dgk_pub'] = np.load(
            "./pub.npy", allow_pickle=True).item()
        current_app.config['dgk_priv'] = np.load(
            "./priv.npy", allow_pickle=True).item()
...
def dgk_compare_has_priv_without_route():
    ...
    # get bits
    y_b = get_bits(
        current_app.config["data"]['compare_operand'],
        current_app.config['dgk_l'])
    # get encrypted bits
    y_enc = []
    for b in y_b:
        if b == 0:
    ...
    response = requests.post(
        "http://127.0.0.1:8000/dgk/compare_no_priv",
        json={
            'y_enc': y_enc})
