...
def veu11_set_pub():
	...
	current_app.config['paillier_pub'] = paillier.PaillierPublicKey(n=mpz(public_key["n"]), g=int(public_key["g"]))
...
def veu11_compare_no_priv():
	...
	x = encrypted_b + paillier.encrypt(mpz(pow(2, l)), current_app.config["paillier_pub"]) - encrypted_a
	...
	response = requests.post("http://127.0.0.1:8000/veu11/compare_has_priv", json={
		"z" : encrypted_z,   
	})
